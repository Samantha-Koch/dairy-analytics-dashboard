...
import { getMilkYieldData, getAvailableYears } from "../../../services/api/productionService";
...
export default function AvgMilkYieldChart({ customerId }: Props) {
  console.log("AvgMilkYieldChart mounted");
  ...
  useEffect(() => {
    getAvailableYears("milk_yield").then((res) => {
  ...
  useEffect(() => {
    console.log("yield useEffect triggered", {source, frequency, year, customerId});
    if (frequency === "monthly" && !year) {
        console.log("yield Blocked because monthly + no year");
        return;
      }
    console.log("yield fetching now");
    getMilkYieldData({
      source,
      frequency,
      year,
  ...
  return (
    <div>
      ...
      <Plot
        data={traces}
        layout={{
          title: "Average Milk Yield Per Cow",
          xaxis: { title: {text: frequency === "annual" ? "Year" : "Month" }},
          yaxis: { title: {text: "Pounds per Cow"} },
          autosize: true,
        }}
